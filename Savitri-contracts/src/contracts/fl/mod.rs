...
mod tests {
    ...
    fn setup() -> (ContractStorage, Storage) {
        let tmp = TempDir::new().unwrap();
        let storage = Storage::new(tmp.path()).unwrap();
        let contract_address = vec![1u8; 32];
        let contract_storage = ContractStorage::new(contract_address).unwrap();
        (contract_storage, storage)
    }

    #[test]
    fn registry_and_rounds_flow() {
        let (mut cs, storage) = setup();
        let owner = [7u8; 32];
        let treasury = [9u8; 32];
        FlAiManager::initialize(&mut cs, &storage, &owner, &treasury, 100, None).unwrap();

        let nft = [5u8; 32];
        let model_id = FlAiManager::register_model(
            &mut cs, &storage, &owner, &nft, 1, [11u8; 32], [12u8; 32], [13u8; 32], [14u8; 32], 1,
            None,
        )
        .unwrap();

        let round_id =
            FlAiManager::create_round(&mut cs, &storage, &owner, model_id, 0, 0, 100, 0, 10, None)
                .unwrap();
        FlAiManager::open_round(&mut cs, &storage, &owner, model_id, round_id, None).unwrap();

        let trainer = [2u8; 32];
        FlAiManager::grant_role(
            &mut cs,
            &storage,
            &owner,
            model_id,
        ...
        .unwrap();
        FlAiManager::submit_update(
            &mut cs, &storage, &trainer, model_id, round_id, [21u8; 32], [22u8; 32], 0, &[0u8; 64],
            2, None,
        )
        .unwrap();
        FlAiManager::seal_round(&mut cs, &storage, &owner, model_id, round_id, None).unwrap();
        let new_version = FlAiManager::finalize_round(
            &mut cs, &storage, &owner, model_id, round_id, [31u8; 32], [32u8; 32], [33u8; 32],
            [34u8; 32], 3, None,
        )
        .unwrap();
    ...
    #[test]
    fn rewards_claim_flow() {
        let (mut cs, storage) = setup();
        let owner = [1u8; 32];
        let treasury = [2u8; 32];
        FlAiManager::initialize(&mut cs, &storage, &owner, &treasury, 100, None).unwrap();
        let model_id = FlAiManager::register_model(
            &mut cs, &storage, &owner, &[3u8; 32], 1, [4u8; 32], [5u8; 32], [6u8; 32], [7u8; 32],
            1, None,
        )
        .unwrap();
        let round_id =
            FlAiManager::create_round(&mut cs, &storage, &owner, model_id, 0, 0, 1, 0, 5, None)
                .unwrap();
        FlAiManager::open_round(&mut cs, &storage, &owner, model_id, round_id, None).unwrap();
        let trainer = [8u8; 32];
        FlAiManager::grant_role(
        ...
        .unwrap();
        FlAiManager::submit_update(
            &mut cs, &storage, &trainer, model_id, round_id, [9u8; 32], [10u8; 32], 0, &[0u8; 64],
            1, None,
        )
        .unwrap();
        FlAiManager::seal_round(&mut cs, &storage, &owner, model_id, round_id, None).unwrap();

        // set roots deterministic = leaf to bypass proof complexity
        let accepted_leaf = keccak_concat(&[
            &model_id.to_le_bytes(),
            &round_id.to_le_bytes(),
            &trainer,
        ...
        let score_leaf = keccak_concat(&[
            ...
            &trainer,
            &score.to_le_bytes(),
            &gross_amount.to_le_bytes(),
        ]);

        FlAiManager::finalize_round(
            &mut cs,
            &storage,
            &owner,
            model_id,
        ...
        let asset = [42u8; 32];
        FlAiManager::fund_pool(&mut cs, &storage, &owner, model_id, asset, 10_000, None).unwrap();

        let (net, fee) = FlAiManager::claim_reward(
            &mut cs,
            &storage,
            &trainer,
            model_id,
