...
mod tests {
    ...
    fn test_tracer_basic_functionality() {
        ...
        let mut tracer = ContractTracer::new(config);
        ...
        let trace_id = tracer
            .start_trace(contract_addr, caller, input.clone(), 0)
            .unwrap();
        ...
        tracer
            .trace_storage_access(
                ...
                Some(vec![7, 8, 9]),
                None,
                StorageAccessType::Read,
                100,
            )
            .unwrap();

        let completed = tracer.complete_trace(vec![10, 11, 12], true, None).unwrap();

        assert_eq!(completed.contract_address, contract_addr);
        assert_eq!(completed.caller, caller);
        assert_eq!(completed.input, input);
        assert_eq!(completed.output, vec![10, 11, 12]);
        assert!(completed.success);
        assert_eq!(completed.storage_accesses.len(), 1);
    }
    ...
    fn test_tracer_disabled() {
        ...
        let trace_id = tracer
            .start_trace([1u8; 32], [2u8; 32], vec![1, 2, 3], 0)
            .unwrap();
        assert!(trace_id.is_empty());
    ...
    fn test_export_json() {
        ...
        tracer
            .start_trace([1u8; 32], [2u8; 32], vec![1, 2, 3], 0)
            .unwrap();
        tracer.complete_trace(vec![10, 11, 12], true, None).unwrap();

        let json = tracer.export_traces().unwrap();
    ...
    fn test_trace_stats() {
        ...
        let stats = tracer.get_trace_stats();
