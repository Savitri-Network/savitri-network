...
mod tests {
    ...
    fn test_compute_tx_root() {
        let tx_hashes: Vec<[u8; 32]> = vec![sha256(b"tx1"), sha256(b"tx2"), sha256(b"tx3")];
        let root = compute_tx_root(&tx_hashes);
    ...
    fn test_verify_leaf() {
        ...
        assert!(tree.verify_leaf(b"a", 0).unwrap());
        assert!(tree.verify_leaf(b"b", 1).unwrap());
        assert!(!tree.verify_leaf(b"x", 0).unwrap());
    }
}
