...
mod tests {
    ...
    fn test_keypair_generation() {
        ...
        // private_key() removed (MED-05): key bytes must not be exposed via public API
    }
    ...
    fn test_sign_message_function() {
        // Generate raw key bytes directly with OsRng for the sign_message() API
        let mut raw_key = [0u8; 32];
        OsRng.fill_bytes(&mut raw_key);
        let signing_key = ed25519_dalek::SigningKey::from_bytes(&raw_key);
        let public_key = signing_key.verifying_key().to_bytes();
        let message = b"test message";
        let signature = sign_message(message, &raw_key).unwrap();
        raw_key.zeroize();
    ...
    fn test_aggregate_signature() {
        ...
        for _ in 0..3 {
            ...
            agg.add(sig, keypair.public_key());
        }

        assert_eq!(agg.len(), 3);
        assert!(agg.verify_all(message).unwrap());
    }
}
