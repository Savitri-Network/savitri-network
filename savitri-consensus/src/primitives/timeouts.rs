...
mod tests {
    ...
    /// Serialize env-mutating tests so they don't race when run in parallel.
    ...
    fn default_values_match_documented_constants() {
        ...
        assert_eq!(t.pending_block_ttl_secs, 900);
        assert_eq!(t.backup_cert_timeout_ms, 2000);
        assert_eq!(t.nonce_window, 3500);
        assert_eq!(t.max_epoch_drift, 100);
        assert_eq!(t.max_pipeline_depth, 16);
        assert_eq!(t.cert_timestamp_tolerance_secs, 900);
        assert_eq!(t.admission_max_main_pool_nonce_gap, 3000);
        assert_eq!(t.inflight_restore_interval_secs, 10);
        assert_eq!(t.inflight_restore_max_age_secs, 30);
    }

    #[test]
    fn env_override_works() {
        let _g = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        std::env::set_var("SAVITRI_PENDING_BLOCK_TTL_SECS", "1234");
        std::env::set_var("SAVITRI_BACKUP_CERT_TIMEOUT_MS", "555");
        std::env::set_var("SAVITRI_NONCE_WINDOW", "7777");
        let t = Timeouts::from_env();
        std::env::remove_var("SAVITRI_PENDING_BLOCK_TTL_SECS");
        std::env::remove_var("SAVITRI_BACKUP_CERT_TIMEOUT_MS");
        std::env::remove_var("SAVITRI_NONCE_WINDOW");
        assert_eq!(t.pending_block_ttl_secs, 1234);
        assert_eq!(t.backup_cert_timeout_ms, 555);
        assert_eq!(t.nonce_window, 7777);
        // Untouched fields still default.
        assert_eq!(t.max_epoch_drift, 100);
    }

    #[test]
    fn invalid_env_keeps_default() {
        let _g = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        std::env::set_var("SAVITRI_MAX_EPOCH_DRIFT", "not-a-number");
        std::env::set_var("SAVITRI_MAX_PIPELINE_DEPTH", "");
        let t = Timeouts::from_env();
        std::env::remove_var("SAVITRI_MAX_EPOCH_DRIFT");
        std::env::remove_var("SAVITRI_MAX_PIPELINE_DEPTH");
        assert_eq!(t.max_epoch_drift, 100);
        assert_eq!(t.max_pipeline_depth, 16);
    }
}
