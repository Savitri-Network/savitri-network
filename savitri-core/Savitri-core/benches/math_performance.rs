...
fn bench_blockchain_operations(c: &mut Criterion) {
    ...
    c.bench_function("compound_interest_12_periods", |bencher| {
        bencher.iter(|| {
            black_box(utils::compound_interest(black_box(principal), black_box(rate), 12));
        });
    });
...
fn bench_real_world_scenarios(c: &mut Criterion) {
    ...
    let validator_data: Vec<(fixed_point::FixedPoint, fixed_point::FixedPoint, fixed_point::FixedPoint, fixed_point::FixedPoint, fixed_point::FixedPoint)> = (1..=100)
        .map(|i| (
    ...
    c.bench_function("pou_validation_100_validators", |bencher| {
        bencher.iter(|| {
            ...
            for (availability, latency, integrity, reputation, participation) in black_box(&validator_data) {
                let weighted_sum = fixed_point::mul(*availability, fixed_point::from_string("0.3").unwrap())
                    + fixed_point::mul(*latency, fixed_point::from_string("0.2").unwrap())
                    + fixed_point::mul(*integrity, fixed_point::from_string("0.2").unwrap())
                    + fixed_point::mul(*reputation, fixed_point::from_string("0.2").unwrap())
