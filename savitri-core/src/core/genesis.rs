...
mod tests {
    ...
    fn test_sign_and_verify() {
        ...
        let verified = verify_signature(data, &signature, key);
    ...
    fn test_decode_hex() {
        ...
        let result = decode_hex::<8>(hex_str);
    ...
    #[test]
    fn test_create_test_genesis_block() {
        let block = create_test_genesis_block();
    ...
    fn test_validate_genesis_block() {
        let block = create_test_genesis_block().unwrap();
        let result = validate_genesis_block(&block);

        assert!(result.is_ok());
    }

    #[test]
    fn test_is_genesis_block() {
        let genesis_block = create_test_genesis_block().unwrap();
        let non_genesis_block = Block {
            height: 1,
            ..genesis_block.clone()
        };

        assert!(is_genesis_block(&genesis_block));
        assert!(!is_genesis_block(&non_genesis_block));
    }

    #[test]
    fn test_create_dev_genesis_block() {
        let block = create_dev_genesis_block();
    ...
    #[test]
    fn test_genesis_addresses() {
        let addresses = get_genesis_addresses();
    ...
    #[test]
    fn test_is_genesis_address() {
        let result = is_genesis_address("0x0000000000000000000000000000000000000001");

        assert!(result.is_ok());
    }

    #[test]
    fn test_export_import_genesis() {
        let block = create_test_genesis_block().unwrap();
        let json = export_genesis_to_json();

        assert!(json.is_ok());
        let json_str = json.unwrap();

        let imported_block = import_genesis_from_json(&json_str);
        assert!(imported_block.is_ok());

        let imported = imported_block.unwrap();
        assert_eq!(block.height, imported.height);
        assert_eq!(block.version, imported.version);
    }
}
