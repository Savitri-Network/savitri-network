...
mod tests {
    ...
    fn create_test_receipt() -> TransactionReceipt {
        ...
        let mut receipt = TransactionReceipt::new(&tx, [0u8; 32]);
        receipt.mark_mempool_entry();
        receipt.mark_block_inclusion(1, 0);
        receipt.mark_consensus();
    ...
    fn test_receipt_lifecycle() {
        ...
        assert!(receipt.get_finality_time().is_some());
        assert!(receipt.get_finality_time().unwrap() > Duration::ZERO);
    }

    #[test]
    fn test_finality_metrics() {
        let receipts: Vec<_> = (0..100).map(|_| create_test_receipt()).collect();
        let metrics = FinalityMetrics::from_receipts(&receipts);

        assert_eq!(metrics.total_transactions, 100);
        assert_eq!(metrics.finalized_transactions, 100);
        assert_eq!(metrics.finality_rate, 1.0);
        assert!(metrics.average_finality_time > Duration::ZERO);
    }
    ...
    fn test_finality_tracker() {
        ...
        // Add receipts
        for _ in 0..50 {
            tracker.add_receipt(create_test_receipt());
        }

        let metrics = tracker.get_metrics();
        assert_eq!(metrics.total_transactions, 50);
        assert_eq!(metrics.finalized_transactions, 50);
        assert_eq!(metrics.finality_rate, 1.0);
    }
}
