...
mod tests {
    ...
    fn test_manifest_generation() {
        ...
        assert_eq!(manifest.platform.name, "Savitri Network");
        assert_eq!(manifest.platform.deployment_type, "decentralized");
        assert!(!manifest.categories.is_empty());
        assert!(!manifest.metrics.is_empty());
        assert!(!manifest.security.principles.is_empty());
    }
    ...
    fn test_manifest_validation() {
        ...
        assert!(ManifestGenerator::validate_manifest(&manifest).is_ok());
    }
    ...
    fn test_manifest_save_load() {
        ...
        // Salva
        assert!(ManifestGenerator::save_manifest(&manifest, temp_path).is_ok());

        // Carica
        let loaded = ManifestGenerator::load_manifest(temp_path).unwrap();
        assert_eq!(manifest.version, loaded.version);
        assert_eq!(manifest.metrics.len(), loaded.metrics.len());
    ...
    fn test_metric_definitions() {
        ...
        // Check che ci siano metriche critical
        let critical_metrics: Vec<_> = manifest
            .metrics
            .iter()
        ...
        let has_counter = manifest
            .metrics
            .iter()
            .any(|m| matches!(m.metric_type, MetricType::Counter));
        let has_gauge = manifest
            .metrics
            .iter()
    ...
    fn test_security_info() {
        ...
        assert!(!manifest.security.principles.is_empty());
        assert!(!manifest.security.access_controls.is_empty());
        assert!(!manifest.security.exposure_limits.is_empty());
        assert!(!manifest.security.recommendations.is_empty());

        // Check principi di sicurezza chiave
        let principles: Vec<_> = manifest
            .security
            .principles
            .iter()
            .filter(|p| p.contains("localhost") || p.contains("external"))
            .collect();
