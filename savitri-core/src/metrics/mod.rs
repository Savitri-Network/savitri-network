...
mod tests {
    ...
    async fn test_metrics_manager_creation() {
        ...
        assert!(manager.get_provider().is_enabled());
    }
    ...
    #[tokio::test]
    async fn test_register_savitri_metrics() {
        let config = MetricsConfig {
            enabled: true,
            ..Default::default()
        };
        ...
        let manager = MetricsManager::new(config);
        manager.register_savitri_metrics().await;

        let stats = manager.get_stats().await;
        assert!(stats.provider_stats.total_metrics > 0);
        assert!(stats.provider_stats.update_count > 0);
    }
    ...
    async fn test_health_checker() {
        ...
        let manager = MetricsManager::new(config);
        let health_status = manager.health_checker.check_health().await;

        assert!(health_status.healthy);
        assert_eq!(health_status.message, "Metrics provider is healthy");
    }
    ...
    async fn test_manifest_generation() {
        ...
        let manager = MetricsManager::new(config);
        let temp_path = "test_metrics_manifest.json";

        // Genera e salva il manifesto
        assert!(manager.generate_and_save_manifest(temp_path).await.is_ok());
    ...
    async fn test_cleanup_old_metrics() {
        ...
        manager
            .register_metric("test_metric".to_string(), 1.0, MetricType::Counter)
            .await;
        ...
        manager.cleanup_old_metrics(Duration::from_millis(5)).await;

        // Check che le metriche siano state rimosse
        let stats = manager.get_stats().await;
        assert_eq!(stats.provider_stats.total_metrics, 0);
    }

    #[test]
    fn test_create_metrics_manager_from_env() {
        // Set variabili d'ambiente
        std::env::set_var("SAVITRI_METRICS_ENABLED", "true");
        std::env::set_var("SAVITRI_METRICS_PORT", "9999");

        let manager = create_metrics_manager_from_env();
        assert!(manager.get_provider().is_enabled());
        assert_eq!(manager.get_provider().get_port(), 9999);
    ...
    #[test]
    fn test_create_default_metrics_manager() {
        let manager = create_default_metrics_manager();
        assert!(!manager.get_provider().is_enabled()); // Default è disabled
        assert_eq!(manager.get_provider().get_port(), 9090);
    }
    ...
    async fn test_manager_stats() {
        ...
        assert_eq!(stats.provider_stats.total_metrics, 0);
        assert!(stats.exporter_stats.is_none());
