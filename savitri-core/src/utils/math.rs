...
mod tests {
    ...
    fn test_constants() {
        ...
        assert!((GOLDEN_RATIO - 1.618033988749895).abs() < 1e-10);
    }

    #[test]
    fn test_basic_operations() {
        assert_eq!(basic::min(5, 3), 3);
        assert_eq!(basic::max(5, 3), 5);
        assert_eq!(basic::clamp(7, 1, 5), 5);
        assert_eq!(basic::clamp(0, 1, 5), 1);
        assert_eq!(basic::clamp(3, 1, 5), 3);
    }

    #[test]
    fn test_power_functions() {
        assert!((power::pow(2.0, 3.0) - 8.0).abs() < 1e-10);
        assert!((power::powi(2.0, 3) - 8.0).abs() < 1e-10);
        assert!(power::sqrt(9.0).unwrap() - 3.0 < 1e-10);
        assert!((power::cbrt(27.0) - 3.0).abs() < 1e-10);
        assert!((power::ln(consts::E) - 1.0).abs() < 1e-10);
        assert!((power::log10(100.0) - 2.0).abs() < 1e-10);
        assert!((power::log2(8.0) - 3.0).abs() < 1e-10);
        assert!((power::exp(1.0) - consts::E).abs() < 1e-10);
    }

    #[test]
    fn test_trigonometry() {
        let angle = trigonometry::deg_to_rad(45.0);
        assert!((trigonometry::sin(angle) - 0.7071067811865475).abs() < 1e-10);
        assert!((trigonometry::cos(angle) - 0.7071067811865476).abs() < 1e-10);
        assert!((trigonometry::tan(angle) - 1.0).abs() < 1e-10);

        assert!((trigonometry::rad_to_deg(consts::PI / 4.0) - 45.0).abs() < 1e-10);
    }

    #[test]
    fn test_rounding() {
        assert_eq!(rounding::round(3.7), 4.0);
        assert_eq!(rounding::floor(3.7), 3.0);
        assert_eq!(rounding::ceil(3.2), 4.0);
        assert_eq!(rounding::trunc(3.7), 3.0);
        assert_eq!(rounding::trunc(-3.7), -3.0);
    }
    ...
    fn test_financial() {
        ...
        let compound = financial::compound_interest(principal, rate, periods, 1);
        let simple = financial::simple_interest(principal, rate, periods);

        assert!(compound > simple); // Compound should be greater than simple

        let fv = financial::future_value(principal, rate, periods);
        let pv = financial::present_value(fv, rate, periods);
        assert!((pv - principal).abs() < 1e-10);
    }
}
