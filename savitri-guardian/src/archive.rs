...
mod tests {
    ...
    fn test_archive_metrics() {
        ...
        let storage = open_archive(temp_dir.path()).unwrap();

        let metrics = get_archive_metrics(&storage).unwrap();
        assert!(metrics.size_gb >= 0.0);
        assert_eq!(metrics.block_count, 1); // Genesis block
        assert_eq!(metrics.monolith_count, 0);
    }
    ...
    fn test_block_storage() {
        ...
        store_block(&storage, block.clone()).unwrap();

        let retrieved = get_block(&storage, 1).unwrap();
        assert!(retrieved.is_some());
        assert_eq!(retrieved.unwrap().height, 1);
    }

    #[test]
    fn test_account_indexing() {
        let temp_dir = TempDir::new().unwrap();
        let storage = open_archive(temp_dir.path()).unwrap();
        ...
        index_account_tx(&storage, &account, 100, &tx_hash).unwrap();

        let history = get_account_history(&storage, &account, None).unwrap();
    ...
    #[test]
    fn test_growth_rate() {
        let old = ArchiveMetrics {
            size_bytes: 1024 * 1024 * 1024, // 1GB
            size_gb: 1.0,
            block_count: 1000,
            monolith_count: 10,
            chain_height: Some(1000),
            measured_at: 1000,
        };

        let new = ArchiveMetrics {
            size_bytes: 2 * 1024 * 1024 * 1024, // 2GB
            size_gb: 2.0,
            block_count: 2000,
            monolith_count: 20,
            chain_height: Some(2000),
            measured_at: 1000 + 86400, // 1 day later
        };

        let rate = calculate_growth_rate(&old, &new).unwrap();
        assert_eq!(rate, 1.0); // 1GB per day
    }
}
