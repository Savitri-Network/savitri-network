...
mod tests {
    ...
    fn create_test_service() -> (LatencyService, mpsc::Receiver<LatencyGossipMessage>) {
        let keypair = Keypair::generate(&mut OsRng);
        let config = LatencyServiceConfig::default();
        let (probe_tx, probe_rx) = mpsc::channel(100);
        let (_msg_tx, msg_rx) = mpsc::channel(100);

        let service = LatencyService::new(keypair, config, probe_tx, msg_rx);
        (service, probe_rx)
    }
    ...
    async fn test_service_creation() {
        ...
        assert!(service.get_latest_measurement().await.is_none());
        assert_eq!(service.get_latency_score().await, 0.0);
    }
    ...
    async fn test_peer_update() {
        ...
        let peers = vec![
            ...
            LatencyPeer {
                ...
                pubkey: [2u8; 32],
                last_rtt_ms: Some(50.0),
                response_rate: 0.9,
            },
        ];

        service.update_peers(peers).await;

        let stored_peers = service.peers.read().await;
    ...
    async fn test_insufficient_peers() {
        ...
        // Only add 5 peers (less than MIN_PROBE_PEERS = 10)
        let peers: Vec<LatencyPeer> = (0..5)
            .map(|i| LatencyPeer {
        ...
        // Measurement should fail due to insufficient peers
        let result = service.measure_latency(1).await;
