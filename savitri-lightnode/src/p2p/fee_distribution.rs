...
mod tests {
    ...
    #[test]
    fn test_calculate_contribution_weight() {
        let peer_info = crate::p2p::types::PeerInfo {
            account: [0u8; 32],
        ...
        let weight = calculate_contribution_weight(7500, &peer_info); // 75% PoU
        assert_eq!(weight, 0.75 * 0.7 + 1.0 * 0.3); // 0.525 + 0.3 = 0.825
    }
    ...
    fn test_fee_distribution() {
        ...
        let distribution = calculate_fee_distribution(1000, 500, &p2p_nodes, 10);

        assert_eq!(distribution.proposer_fee, 1000);
        assert_eq!(distribution.total_distributed, 1500);
        assert_eq!(distribution.p2p_fees.len(), 2);
    }
}
