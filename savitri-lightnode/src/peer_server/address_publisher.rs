...
mod tests {
    ...
    use super::{build_public_rpc_endpoint, compute_publishable_addresses, AddressPublishOptions};

    #[test]
    fn excludes_private_addresses_by_default() {
        let listen_addrs: Vec<Multiaddr> = vec![
            "/ip4/127.0.0.1/tcp/4001".parse().unwrap(),
            "/ip4/10.0.0.5/tcp/4001".parse().unwrap(),
            "/ip4/198.51.100.10/tcp/4001".parse().unwrap(),
        ];
        let observed: Multiaddr = "/ip4/203.0.113.8/tcp/4001".parse().unwrap();

        let published = compute_publishable_addresses(
            &listen_addrs,
            Some(&observed),
            &AddressPublishOptions::default(),
        );

        assert_eq!(
            published,
            vec![
                "/ip4/203.0.113.8/tcp/4001".parse().unwrap(),
                "/ip4/198.51.100.10/tcp/4001".parse().unwrap(),
            ]
        );
    }
    ...
    #[test]
    fn can_publish_private_addresses_when_enabled() {
        let listen_addrs: Vec<Multiaddr> = vec!["/ip4/10.0.0.5/tcp/4001".parse().unwrap()];
        let published = compute_publishable_addresses(
            &listen_addrs,
            None,
            &AddressPublishOptions {
                publish_private_addresses: true,
                rpc_port: None,
                rpc_bind_addr: None,
            },
        );
        assert_eq!(published, listen_addrs);
    }

    #[test]
    fn builds_rpc_endpoint_from_publishable_address() {
        let addrs: Vec<Multiaddr> = vec!["/ip4/198.51.100.10/tcp/4001".parse().unwrap()];
        let rpc = build_public_rpc_endpoint(
            &addrs,
            &AddressPublishOptions {
                publish_private_addresses: false,
                rpc_port: Some(8545),
                rpc_bind_addr: Some("0.0.0.0".to_string()),
            },
        );
        assert_eq!(rpc.as_deref(), Some("http://198.51.100.10:8545"));
    }
}
