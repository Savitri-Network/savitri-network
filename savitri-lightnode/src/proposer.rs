...
mod tests {
    ...
    #[tokio::test]
    async fn test_pou_score_calculation() {
        let mut state = PoUScoreState::default();

        state.update_latency(0.8);
        state.update_integrity(0.9);
        state.update_reputation(0.7);

        // Expected: 0.4 * 0.8 + 0.3 * 0.9 + 0.3 * 0.7 = 0.32 + 0.27 + 0.21 = 0.80
        assert_eq!(state.total_score, 8000);
    }

    #[tokio::test]
    async fn test_proposer_service_creation() {
        let keypair = create_test_keypair();
        let config = ProposerConfig::default();
        let (tx, _rx) = mpsc::channel(10);

        let service = ProposerService::new(keypair, config, tx);
        assert_eq!(service.get_pou_score().await, 0);
    }

    #[tokio::test]
    async fn test_election_with_candidates() {
        let keypair = create_test_keypair();
        let config = ProposerConfig::default();
        let (tx, _rx) = mpsc::channel(10);
        ...
        // Set our score
        service.update_latency_score(0.8).await;
        service.update_masternode_scores(0.9, 0.7).await;

        // Add a peer with lower score
        service
            .update_peer_score(
                [1u8; 32],
                ProposerCandidate {
        ...
        let election = service.run_election(1).await;

        // We should win with score 8000 vs 5000
        assert!(election.is_local_winner);
        assert_eq!(election.winner_pou_score, 8000);
    }
    ...
    async fn test_eligibility_check() {
        ...
        // Initially not eligible (score = 0)
        assert!(!service.is_eligible().await);

        // After updating scores
        service.update_latency_score(0.8).await;
        service.update_masternode_scores(0.9, 0.7).await;

        // Now eligible (score = 8000 > 5000)
        assert!(service.is_eligible().await);
    }
}
