...
mod tests {
    ...
    #[tokio::test]
    async fn cache_hit_after_announce() {
        let cache = ProposerCache::new(Duration::from_secs(30));
        let peer = random_peer();
        cache
            .update_from_announce("group_a".to_string(), peer, None)
            .await;
        let entry = cache.try_get("group_a").expect("should hit");
        assert_eq!(entry.peer_id, peer);
        assert_eq!(entry.source, ProposerSource::GroupAnnounce);
    }

    #[tokio::test]
    async fn cache_miss_for_unknown_group() {
        let cache = ProposerCache::default();
        assert!(cache.try_get("unknown").is_none());
    }

    #[tokio::test]
    async fn ttl_expiry_returns_stale() {
        let cache = ProposerCache::new(Duration::from_millis(10));
        let peer = random_peer();
        cache
            .update_from_announce("group_b".to_string(), peer, None)
            .await;
        tokio::time::sleep(Duration::from_millis(20)).await;
        let entry = cache.try_get("group_b").expect("should still return stale");
        assert_eq!(entry.source, ProposerSource::Stale);
    }

    #[tokio::test]
    async fn announce_overwrites_kad_entry() {
        let cache = ProposerCache::default();
        let peer_kad = random_peer();
        let peer_ann = random_peer();
        cache
            .update_from_kad("group_c".to_string(), peer_kad, None)
            .await;
        cache
            .update_from_announce("group_c".to_string(), peer_ann, None)
            .await;
        let entry = cache.try_get("group_c").expect("should hit");
        assert_eq!(entry.peer_id, peer_ann);
        assert_eq!(entry.source, ProposerSource::GroupAnnounce);
    }

    #[tokio::test]
    async fn kad_does_not_overwrite_fresh_announce() {
        let cache = ProposerCache::new(Duration::from_secs(30));
        let peer_ann = random_peer();
        let peer_kad = random_peer();
        cache
            .update_from_announce("group_d".to_string(), peer_ann, None)
            .await;
        cache
            .update_from_kad("group_d".to_string(), peer_kad, None)
            .await;
        let entry = cache.try_get("group_d").expect("should hit");
        assert_eq!(entry.peer_id, peer_ann, "announce wins over kad when fresh");
    }

    #[tokio::test]
    async fn mark_stale_flags_existing_entry() {
        let cache = ProposerCache::default();
        let peer = random_peer();
        cache
            .update_from_announce("group_e".to_string(), peer, None)
            .await;
        cache.mark_stale("group_e").await;
        let entry = cache.try_get("group_e").expect("should hit");
        assert_eq!(entry.source, ProposerSource::Stale);
    }
}
