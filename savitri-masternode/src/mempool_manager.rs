...
mod tests {
    ...
    fn create_test_entry(id: u32) -> MempoolEntry {
        MempoolEntry {
    ...
    fn test_add_transaction() {
        ...
        assert!(manager.add_transaction(entry).unwrap());
        assert!(manager.contains(&tx_hash));
        assert_eq!(manager.pending_count(), 1);
    }

    #[test]
    fn test_confirm_transactions() {
        let mut manager = MempoolManager::new();

        // Add transactions
        for i in 1..=5 {
            let entry = create_test_entry(i);
            manager.add_transaction(entry).unwrap();
        }

        assert_eq!(manager.pending_count(), 5);

        // Confirm some transactions
        let confirmed_hashes = vec![create_hash_from_id(1), create_hash_from_id(2)];

        let confirmed_count = manager.confirm_transactions(&confirmed_hashes);

        assert_eq!(confirmed_count, 2);
        assert_eq!(manager.pending_count(), 3);
        assert_eq!(
            manager.get_status(&create_hash_from_id(1)),
            MempoolStatus::Confirmed
        );
    }

    #[test]
    fn test_reject_transactions() {
        let mut manager = MempoolManager::new();
        ...
        let rejected_hashes = vec![create_hash_from_id(3)];
        let rejected_count = manager.reject_transactions(&rejected_hashes);

        assert_eq!(rejected_count, 1);
        assert_eq!(
            manager.get_status(&create_hash_from_id(3)),
            MempoolStatus::Rejected
        );
    }
    ...
    fn test_mempool_sync() {
        ...
        let confirmed = vec![create_hash_from_id(1), create_hash_from_id(2)];
        let rejected = vec![create_hash_from_id(3)];

        let result = manager.process_mempool_sync(&confirmed, &rejected);

        assert_eq!(result.confirmed_count, 2);
        assert_eq!(result.rejected_count, 1);
        assert_eq!(result.remaining_pending, 7);
    }
    ...
    fn test_duplicate_rejection() {
        ...
        // Add and confirm a transaction
        let entry = create_test_entry(1);
        let tx_hash = entry.tx_hash;
        manager.add_transaction(entry).unwrap();
        manager.confirm_transactions(&[tx_hash]);

        // Try to add same transaction again
        let entry2 = create_test_entry(1);
        let result = manager.add_transaction(entry2);

        assert!(matches!(result, Err(MempoolError::AlreadyConfirmed)));
    }
    ...
    fn test_get_transactions_for_block() {
        ...
        for i in 1..=10 {
            ...
            entry.priority = i;
            entry.gas_price = i as u64 * 100;
            manager.add_transaction(entry).unwrap();
        }

        // Get top 5 transactions
        let txs = manager.get_transactions_for_block(5);

        assert_eq!(txs.len(), 5);
        // Should be sorted by priority (highest first)
        assert!(txs[0].priority >= txs[1].priority);
    }
}
