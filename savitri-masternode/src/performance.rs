...
mod tests {
    ...
    #[test]
    fn test_lru_cache() {
        let mut cache: TransactionLRUCache<u64> = TransactionLRUCache::new(3);

        cache.insert([1u8; 32], 100);
        cache.insert([2u8; 32], 200);
        cache.insert([3u8; 32], 300);

        assert_eq!(cache.get(&[1u8; 32]), Some(&100));
        assert_eq!(cache.len(), 3);

        // Insert one more, should evict LRU
        cache.insert([4u8; 32], 400);
        assert_eq!(cache.len(), 3);
    }
    ...
    fn test_bloom_filter() {
        ...
        // tx3 might return true (false positive) or false
    }

    #[test]
    fn test_batch_processor() {
        let mut processor = BatchProcessor::new(100, 1000);
