...
mod tests {
    ...
    fn test_signature_verifier_creation() {
        ...
        let stats = verifier.get_cache_stats();
        assert_eq!(stats.cached_keys, 0);
    }

    #[test]
    fn test_aggregate_signature_verifier_with_real_keys() {
        use ed25519_dalek::{Signer, SigningKey};

        let verifier = AggregateSignatureVerifier::new();
        let message_hash = [1u8; 32];

        // Generate real Ed25519 keypairs and sign the message
        ...
        let voter_signatures = vec![sig1.to_bytes(), sig2.to_bytes()];
        let aggregated = AggregateSignatureVerifier::aggregate_signatures(&voter_signatures);

        let result = verifier.verify_aggregate_signature(
            &message_hash,
            &voter_pubkeys,
            &voter_signatures,
            &aggregated,
        ...
        assert!(result.is_ok());
        assert!(result.unwrap()); // Real signatures should verify
    }

    #[test]
    fn test_aggregate_signature_rejects_forged_signatures() {
        let verifier = AggregateSignatureVerifier::new();
        let message_hash = [1u8; 32];

        // Use fake pubkeys and signatures (should fail individual Ed25519 verification)
        let voter_pubkeys = vec![[2u8; 32], [3u8; 32]];
        let voter_signatures = vec![[4u8; 64], [5u8; 64]];
        let aggregated = AggregateSignatureVerifier::aggregate_signatures(&voter_signatures);
        ...
        assert!(result.is_ok());
        assert!(!result.unwrap()); // Forged signatures should fail
    }

    #[test]
    fn test_aggregate_signature_empty_voters() {
        let verifier = AggregateSignatureVerifier::new();

        let message_hash = [1u8; 32];
        let result = verifier.verify_aggregate_signature(&message_hash, &[], &[], &[0u8; 64]);

        assert!(result.is_ok());
        assert!(!result.unwrap()); // Should return false for empty voters
    }

    #[test]
    fn test_aggregate_signature_rejects_zero_signatures() {
        let verifier = AggregateSignatureVerifier::new();
        let message_hash = [1u8; 32];

        let voter_pubkeys = vec![[2u8; 32]];
        let voter_signatures = vec![[0u8; 64]]; // Zero signature
        let aggregated = [0u8; 64];
        ...
        assert!(result.is_ok());
        assert!(!result.unwrap()); // Zero signatures should be rejected
    }
}
