...
mod tests {
    ...
    fn create_test_tx(sender_id: u32, nonce: u64, fee: u64, received_at: Instant) -> MempoolTx {
        MempoolTx::new_with_security_fields(
            sender_id,
            vec![sender_id as u8; 32],
            nonce,
            fee,
            fee * 2,
            21_000,
            crate::mempool::types::TxHandle(sender_id as u64),
            TxClass::Financial,
            [sender_id as u8; 64],
            None,
        )
    ...
    impl MempoolTx {
        fn with_received_at(mut self, received_at: Instant) -> Self {
    ...
    #[test]
    fn test_fee_priority_resolution() {
        let mut resolver =
            NonceConflictResolver::with_strategy(ConflictResolutionStrategy::FeePriority);

        let now = Instant::now();
        let transactions = vec![
            create_test_tx(1, 0, 100, now), // Lower fee
            create_test_tx(1, 0, 300, now + std::time::Duration::from_millis(10)), // Higher fee
            create_test_tx(1, 0, 200, now), // Medium fee
        ];

        let winners = resolver.resolve_conflicts(&transactions);

        assert_eq!(winners.len(), 1);
        assert_eq!(transactions[winners[0]].fee, 300); // Highest fee wins
    }

    #[test]
    fn test_temporal_priority_resolution() {
        let mut resolver =
            NonceConflictResolver::with_strategy(ConflictResolutionStrategy::TemporalPriority);

        let base_time = Instant::now();
        let transactions = vec![
            create_test_tx(1, 0, 100, base_time + std::time::Duration::from_millis(10)), // Later
            create_test_tx(1, 0, 200, base_time), // Earlier (wins)
            create_test_tx(1, 0, 300, base_time + std::time::Duration::from_millis(5)), // Later
        ];

        let winners = resolver.resolve_conflicts(&transactions);

        assert_eq!(winners.len(), 1);
        assert_eq!(transactions[winners[0]].received_at, base_time); // Earlier wins
    }

    #[test]
    fn test_no_conflicts() {
        let mut resolver = NonceConflictResolver::new();

        let transactions = vec![
            create_test_tx(1, 0, 100, Instant::now()),
            create_test_tx(1, 1, 200, Instant::now()),
            create_test_tx(2, 0, 150, Instant::now()),
        ];

        let winners = resolver.resolve_conflicts(&transactions);

        assert_eq!(winners.len(), 3); // All pass through (no conflicts)
    }

    #[test]
    fn test_conflict_analysis() {
        let resolver = NonceConflictResolver::new();

        let transactions = vec![
            create_test_tx(1, 0, 100, Instant::now()),
            create_test_tx(1, 0, 200, Instant::now()), // Conflict with above
            create_test_tx(1, 1, 150, Instant::now()),
            create_test_tx(2, 0, 100, Instant::now()),
            create_test_tx(2, 0, 250, Instant::now()), // Conflict with above
            create_test_tx(2, 0, 300, Instant::now()), // Conflict with above
            create_test_tx(3, 0, 100, Instant::now()),
        ];

        let analysis = resolver.analyze_conflicts(&transactions);

        assert_eq!(analysis.total_transactions, 7);
        assert_eq!(analysis.total_conflicts, 2); // Two conflict groups
        assert_eq!(analysis.max_conflict_size, 3); // One group has 3 transactions
        assert!(analysis.conflict_rate() > 0.0);
        assert!(analysis.has_significant_conflicts());
    }
}
