...
mod tests {
    ...
    fn create_test_transaction() -> SignedTx {
        use ed25519_dalek::{Signer, SigningKey};
        use rand::rngs::OsRng;
        ...
        // Create transaction with proper signature field size (32 bytes for Ed25519)
        let tx = SignedTx {
            from: public_key.as_bytes().to_vec(),
            to: [1u8; 32].to_vec(),
            amount: 1000,
            nonce: 1,
            fee: 100,
            pubkey: public_key.as_bytes().to_vec(),
            sig: [0u8; 32], // Will be replaced with real signature
            pre_verified: false,
        };

        // Sign the transaction using the message method
    ...
    #[test]
    fn test_transaction_validator_creation() -> Result<()> {
        let storage = create_test_storage();
        let config = ValidatorConfig::default();
        let validator = TransactionValidator::new(storage, config);

        let stats = validator.get_stats();
        assert_eq!(stats.replay_cache_size, 0);
        assert_eq!(stats.nonce_cache_size, 0);

        Ok(())
    }

    #[test]
    fn test_signature_validation() -> Result<()> {
        let storage = create_test_storage();
        let validator = TransactionValidator::new_default(storage);
        ...
        assert!(
            result.is_ok(),
            "Valid transaction should pass signature validation"
        );

        // Test invalid signature
        let mut invalid_tx = tx.clone();
        invalid_tx.sig = vec![0u8; 64]; // Invalid signature

        let result = validator.validate_signature(&invalid_tx);
    ...
    fn test_fee_validation() -> Result<()> {
        ...
        // Test valid fee
        let result = validator.validate_fee_bounds(&tx);
        assert!(result.is_ok(), "Valid fee should pass validation");

        // Test fee too low
        tx.fee = 0;
        let result = validator.validate_fee_bounds(&tx);
        assert!(result.is_err(), "Zero fee should fail validation");

        // Test fee too high
        tx.fee = 1000000000; // 1 SAVI
        let result = validator.validate_fee_bounds(&tx);
        assert!(result.is_err(), "Excessive fee should fail validation");

        Ok(())
    }

    #[test]
    fn test_nonce_sequence_validation() -> Result<()> {
        let storage = create_test_storage();
        let validator = TransactionValidator::new_default(storage);

        let tx = create_test_transaction();

        // Test first transaction (nonce 1 should be valid)
        let result = validator.validate_nonce_sequence(&tx);
        assert!(
            result.is_ok(),
            "First transaction with nonce 1 should be valid"
        );

        // Test duplicate nonce
        let result = validator.validate_nonce_sequence(&tx);
        assert!(result.is_err(), "Duplicate nonce should fail validation");

        // Test nonce gap (nonce 3 when nonce 2 expected)
        let mut tx3 = tx.clone();
        tx3.nonce = 3;
        let result = validator.validate_nonce_sequence(&tx3);
        assert!(result.is_err(), "Nonce gap should fail validation");

        Ok(())
    }

    #[test]
    fn test_replay_prevention() -> Result<()> {
        let storage = create_test_storage();
        let validator = TransactionValidator::new_default(storage);

        let tx = create_test_transaction();
        let tx_hash = Sha256::digest(&tx.message());

        let result = validator.validate_transaction(&tx);
        assert!(result.is_ok(), "First transaction should pass");

        let result = validator.validate_transaction(&tx);
        assert!(result.is_err(), "Replay transaction should fail");

        Ok(())
    }

    #[test]
    fn test_public_key_wrapper() -> Result<()> {
        let signing_key = SigningKey::generate(&mut rand::rngs::OsRng);
        let public_key = signing_key.verifying_key();

        // Test valid public key
        let wrapper = DalekPublicKeyWrapper::from_bytes(public_key.as_bytes())?;
        assert!(wrapper.key.as_bytes() == public_key.as_bytes());

        // Test invalid public key
        let result = DalekPublicKeyWrapper::from_bytes(&[0u8; 31]); // Wrong length
    ...
    #[test]
    fn test_signature_wrapper() -> Result<()> {
        let signing_key = SigningKey::generate(&mut rand::rngs::OsRng);
        let message = b"test message";
        let signature = signing_key.sign(message);

        // Test valid signature
        let wrapper = DalekSignatureWrapper::from(&signature.to_bytes().to_vec());
        assert_eq!(wrapper.as_bytes(), signature.as_bytes());

        // Test invalid signature
        let invalid_sig = vec![0u8; 64];
        let wrapper = DalekSignatureWrapper::from(&invalid_sig);
        assert_eq!(wrapper.as_bytes(), &invalid_sig);
    ...
    fn test_transaction_message_serialization() {
        ...
        assert!(
            !message.is_empty(),
            "Transaction message should not be empty"
        );

        // Message should be deterministic
        let message2 = tx.message();
        assert_eq!(
            message, message2,
            "Transaction message should be deterministic"
        );

        // Message should include all transaction fields
        assert!(
            message.len() > 32,
            "Message should include all transaction fields"
        );
