...
mod tests {
    ...
    #[test]
    fn test_mempool_metrics_basic() {
        // Test mempool size
        set_mempool_size(1000);
        assert_eq!(get_mempool_size(), 1000);

        // Test pending tx count
        set_pending_tx_count(500);
        assert_eq!(get_pending_tx_count(), 500);

        // Test ready tx count
        set_ready_tx_count(500);
        assert_eq!(get_ready_tx_count(), 500);

        // Test admission/rejection rates
        increment_admission();
        increment_admission();
        increment_rejection();
        let admission_rate = get_admission_rate();
        let rejection_rate = get_rejection_rate();
        assert!(admission_rate > 0.0 && admission_rate <= 100.0);
        assert!(rejection_rate > 0.0 && rejection_rate <= 100.0);
        assert!((admission_rate + rejection_rate - 100.0).abs() < 0.01);

        // Test eviction
        increment_eviction();
        increment_eviction();
        assert_eq!(get_eviction_count(), 2);

        println!("✅ Mempool metrics basic functionality test passed");
    }

    #[test]
    fn test_update_mempool_metrics() {
        update_mempool_metrics(2000, 800, 1200);
        assert_eq!(get_mempool_size(), 2000);
        assert_eq!(get_pending_tx_count(), 800);
        assert_eq!(get_ready_tx_count(), 1200);

        println!("✅ Update mempool metrics test passed");
    }

    #[test]
    fn test_admission_rejection_rates() {
        // Reset counters
        ADMISSIONS.store(0, Ordering::Relaxed);
        REJECTIONS.store(0, Ordering::Relaxed);

        // Test with only admissions
        increment_admission();
        increment_admission();
        assert_eq!(get_admission_rate(), 100.0);
        assert_eq!(get_rejection_rate(), 0.0);

        // Test with admissions and rejections
        increment_rejection();
        let admission_rate = get_admission_rate();
        let rejection_rate = get_rejection_rate();
        assert!(admission_rate > 0.0 && admission_rate < 100.0);
        ...
        println!("✅ Admission/rejection rates test passed");
    }
    ...
    fn test_counter_getters() {
        ADMISSIONS.store(0, Ordering::Relaxed);
        REJECTIONS.store(0, Ordering::Relaxed);
        REMOVALS.store(0, Ordering::Relaxed);

        increment_admission();
        increment_admission();
        increment_rejection();
        increment_removal_batch(3);

        assert_eq!(get_admission_count(), 2);
        assert_eq!(get_rejection_count(), 1);
        assert_eq!(get_removal_count(), 3);
    }
}
