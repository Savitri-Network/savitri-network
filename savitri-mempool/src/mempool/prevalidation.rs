...
mod tests {
    ...
    #[tokio::test]
    async fn non_oracle_prevalidation_remains_valid() -> anyhow::Result<()> {
        // Temp storage
        let nanos = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)?
            .as_nanos();
        let tmp_dir = std::env::temp_dir().join(format!("savitri-preval-regression-{}", nanos));
        fs::create_dir_all(&tmp_dir)?;
        let storage = Arc::new(Storage::new(tmp_dir.to_str().unwrap())?);

        // Build a normal SignedTx (non-oracle) with valid signature
        ...
        let sig = sign_data(&kp, &tx.message());
        ...
        let prevalidator = Prevalidator::new(storage);
        let results = prevalidator.prevalidate_batch(vec![raw]).await;

        assert_eq!(results.len(), 1);
        match &results[0] {
            PrevalidationResult::Valid(pv) => {
                // Should stay non-System (financial path), proving oracle logic doesn't interfere
                assert!(!matches!(pv.class, TxClass::System));
            }
            other => panic!("expected valid non-oracle tx, got {:?}", other),
        }
