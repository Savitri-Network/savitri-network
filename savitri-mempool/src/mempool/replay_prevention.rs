...
/// Unit tests for replay prevention
#[cfg(test)]
mod tests {
    use super::*;
    ...
    #[tokio::test]
    async fn test_replay_prevention_basic() -> anyhow::Result<()> {
        let (storage, _tmp_dir) = test_utils::create_test_storage("test_replay_basic")?;
        let storage = Arc::new(storage);
        let replay = ReplayPrevention::with_storage(storage.clone());

        let keypair = generate_keypair();
        let sender_address = keypair.public.to_bytes();
        let tx_hash = [42u8; 64];
        let nonce = 0;

        // Initially should not be a replay
        assert!(!replay.is_replay_transaction(&tx_hash, &sender_address, nonce));

        // Record transaction
        replay.record_transaction_execution(tx_hash, sender_address.to_vec(), nonce, 1)?;

        // Now should be a replay
        assert!(replay.is_replay_transaction(&tx_hash, &sender_address, nonce));
    ...
    #[tokio::test]
    async fn test_nonce_validation() -> anyhow::Result<()> {
        let (storage, _tmp_dir) = test_utils::create_test_storage("test_nonce_validation")?;
        let storage = Arc::new(storage);
        let replay = ReplayPrevention::with_storage(storage.clone());
        ...
        storage.put_account(&sender_address, &account.encode())?;

        // Valid nonce (current + 1)
        assert!(replay
            .validate_transaction_nonce(&sender_address, 6)
            .is_ok());

        // Invalid nonce (too old)
        assert!(replay
            .validate_transaction_nonce(&sender_address, 4)
            .is_err());

        // Invalid nonce (too far ahead)
        assert!(replay
            .validate_transaction_nonce(&sender_address, 7)
            .is_err());
    ...
    #[tokio::test]
    async fn test_cleanup_functionality() -> anyhow::Result<()> {
        let config = ReplayPreventionConfig {
            max_hash_cache_size: 10,
            max_nonce_cache_size: 5,
            cache_ttl_blocks: 100,
            cleanup_interval_blocks: 50,
        };

        let (storage, _tmp_dir) = test_utils::create_test_storage("test_cleanup")?;
        let storage = Arc::new(storage);
        let replay = ReplayPrevention::new(config, storage.clone());
        ...
        // Add some entries
        for i in 0..15 {
            let tx_hash = [i as u8; 64];
            replay.record_transaction_execution(tx_hash, sender_address.to_vec(), i, i)?;
        }

        // Should have entries
        assert_eq!(replay.get_stats().hash_cache_size, 15);

        // Update block height to trigger cleanup
        replay.update_block_height(150);

        // Should have cleaned up old entries
        assert!(replay.get_stats().hash_cache_size <= 10);
