...
mod tests {
    ...
    fn test_rpc_error_creation() {
        ...
        let error_with_data =
            RpcError::with_data(-32602, "Invalid params", json!({"field": "value"}));
        assert_eq!(error_with_data.code, -32602);
        assert!(error_with_data.data.is_some());
    }

    #[tokio::test]
    async fn test_simple_rpc_server() {
        let config = RpcConfig::default();
        let mut server = SimpleRpcServer::new(config).unwrap();

        // Register methods
        let node_info_method = SimpleRpcMethod::GetNodeInfo {
            node_info: json!({
                "version": "1.0.0",
                "protocol": "savitri"
            }),
        ...
        let peer_count_method = SimpleRpcMethod::GetPeerCount { peer_count: 42 };

        assert!(server.register_method(node_info_method).is_ok());
        assert!(server.register_method(peer_count_method).is_ok());

        // Test method registration
        assert!(server.is_method_registered("savitri_getNodeInfo"));
        assert!(server.is_method_registered("savitri_getPeerCount"));
    ...
    #[test]
    fn test_simple_rpc_method() {
        let method = SimpleRpcMethod::GetNodeInfo {
            node_info: json!({
                "version": "1.0.0",
                "protocol": "savitri"
            }),
