...
mod tests {
    ...
    fn test_gossip_config_default() {
        ...
        assert_eq!(config.max_cache_size, GOSSIP_CACHE_MAX_SIZE);
        assert_eq!(config.max_message_age, GOSSIP_CACHE_MAX_AGE);
        assert_eq!(config.max_rounds, GOSSIP_MAX_ROUNDS);
    }

    #[test]
    fn test_network_state_height_tracking() {
        let mut state = NetworkState::default();

        state.update_height(100);
        assert_eq!(state.current_height, 100);
        assert!(!state.should_cleanup_by_height());

        state.update_height(201); // Beyond max height diff
        assert!(state.should_cleanup_by_height());

        state.mark_cleanup_done();
        assert_eq!(state.last_cleanup_height, 201);
        assert!(!state.should_cleanup_by_height());
    }
    ...
    fn test_memory_usage_calculation() {
        ...
        let memory_mb = usage.calculate_memory_mb();
