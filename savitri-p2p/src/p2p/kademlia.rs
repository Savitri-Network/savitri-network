...
mod tests {
    ...
    fn test_kademlia_creation() {
        ...
        let dht = KademliaDht::new(&keypair, config);

        assert!(!dht.is_bootstrapped());
        assert_eq!(*dht.local_peer_id(), PeerId::from(keypair.public()));
        assert_eq!(dht.stats().queries_sent, 0);
    }

    #[test]
    fn test_kademlia_add_peer() {
        let keypair = Keypair::generate_ed25519();
        let config = KademliaConfig::default();
        let mut dht = KademliaDht::new(&keypair, config);

        let peer_id = PeerId::random();
        let addr: Multiaddr = "/ip4/127.0.0.1/tcp/4001".parse().unwrap();
        dht.add_peer(peer_id, addr.clone());

        assert!(dht.known_peers().contains_key(&peer_id));
        assert_eq!(dht.stats().peers_discovered, 1);
    }

    #[test]
    fn test_kademlia_random_walk_scheduling() {
        let keypair = Keypair::generate_ed25519();
        let config = KademliaConfig {
            random_walk_interval: Duration::from_millis(50),
            ..Default::default()
        };
        let mut dht = KademliaDht::new(&keypair, config);

        // Initially should want to walk
        assert!(dht.should_random_walk());

        dht.random_walk();
        assert_eq!(dht.stats().random_walks_performed, 1);

        // Just walked, shouldn't need another one yet
        assert!(!dht.should_random_walk());
    }

    #[test]
    fn test_kademlia_bootstrap_no_peers() {
        let keypair = Keypair::generate_ed25519();
        let config = KademliaConfig::default();
        let mut dht = KademliaDht::new(&keypair, config);

        // Bootstrap with no peers should succeed (warn only)
        let result = dht.bootstrap();
        assert!(result.is_ok());
        assert_eq!(dht.stats().bootstrap_attempts, 1);
    }

    #[test]
    fn test_kademlia_with_bootstrap_peers() {
        let keypair = Keypair::generate_ed25519();
        let peer1 = PeerId::random();
        let addr1: Multiaddr = "/ip4/10.0.0.1/tcp/4001".parse().unwrap();
        let peer2 = PeerId::random();
        let addr2: Multiaddr = "/ip4/10.0.0.2/tcp/4001".parse().unwrap();

        let config = KademliaConfig {
            bootstrap_peers: vec![(peer1, addr1), (peer2, addr2)],
            ..Default::default()
        };
        let dht = KademliaDht::new(&keypair, config);

        // Bootstrap peers should be in the known peers via behaviour
        // (actual routing table populated after connection)
        assert_eq!(dht.stats().peers_discovered, 0); // add_peer not called manually
    }
}
