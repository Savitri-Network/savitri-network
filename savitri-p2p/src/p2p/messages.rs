...
mod tests {
    ...
    fn test_message_creation() {
        let message = Message::new(MessageType::Ping, "peer1".to_string(), b"ping".to_vec());

        assert_eq!(message.message_type, MessageType::Ping);
        assert_eq!(message.source_peer, "peer1");
        assert_eq!(message.target_peer, None);
    ...
    fn test_message_with_target() {
        let message = Message::new(MessageType::Direct, "peer1".to_string(), b"hello".to_vec())
            .with_target("peer2".to_string());

        assert_eq!(message.target_peer, Some("peer2".to_string()));
        assert!(!message.is_broadcast());
    }
    ...
    async fn test_message_router() {
        ...
        // Add ping handler
        router.add_handler(Box::new(PingHandler));

        // Create ping message
        let ping_message = Message::new(MessageType::Ping, "peer1".to_string(), b"ping".to_vec());

        // Route message
        let responses = router.route_message(ping_message).await.unwrap();

        // Check stats
        let stats = router.get_stats();
        assert_eq!(stats.messages_received, 1);
        assert_eq!(stats.messages_by_type.get(&MessageType::Ping), Some(&1));
    }
