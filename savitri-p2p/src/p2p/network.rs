...
mod tests {
    ...
    fn test_connection_info() {
        let mut connection =
            ConnectionInfo::new("peer1".to_string(), "127.0.0.1:8333".to_string(), true);

        assert_eq!(connection.peer_id, "peer1");
        assert_eq!(connection.address, "127.0.0.1:8333");
        assert!(connection.is_outgoing);
        assert_eq!(connection.messages_sent, 0);
        assert!(!connection.is_stale(Duration::from_secs(1)));
    ...
    async fn test_peer_connection() {
        ...
        assert!(manager
            .connect_to_peer("peer1".to_string(), "127.0.0.1:8334".to_string())
            .await
            .is_ok());

        // Check connection exists
        assert!(manager.get_connection("peer1").is_some());
        assert_eq!(manager.get_connected_peers().len(), 1);

        // Test sending message
        assert!(manager
            .send_message("peer1".to_string(), b"Hello, World!".to_vec())
            .await
            .is_ok());

        // Test receiving message
        assert!(manager
            .receive_message("peer1".to_string(), b"Hello, back!".to_vec())
            .await
            .is_ok());

        // Test closing connection
        assert!(manager.close_connection("peer1").await.is_ok());
        assert!(manager.get_connection("peer1").is_none());
    ...
    async fn test_connection_limits() {
        ...
        assert!(manager
            ...
            .is_ok());
        assert!(manager
            .connect_to_peer("peer2".to_string(), "127.0.0.1:8335".to_string())
            .await
            .is_ok());

        // Try to connect to one more (should fail)
        assert!(manager
            .connect_to_peer("peer3".to_string(), "127.0.0.1:8336".to_string())
            .await
            .is_err());

        // Try to connect to same peer again (should fail)
        assert!(manager
            .connect_to_peer("peer1".to_string(), "127.0.0.1:8334".to_string())
            .await
            .is_err());
