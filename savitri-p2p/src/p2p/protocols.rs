...
mod tests {
    ...
    fn test_protocol_config_default() {
        ...
        assert_eq!(config.name, "savitri-p2p");
    ...
    async fn test_protocol_handler() {
        ...
        assert!(handler
            .establish_connection("peer1".to_string())
            .await
            .is_ok());
        assert_eq!(handler.get_active_peers().len(), 1);
        assert!(handler.is_connected("peer1"));

        // Test message sending
        let data = b"Hello, World!".to_vec();
        assert!(handler
            .send_message("peer1".to_string(), data.clone())
            .await
            .is_ok());

        // Test message receiving
        assert!(handler
            .receive_message("peer1".to_string(), data)
            .await
            .is_ok());

        // Test connection closing
        assert!(handler.close_connection("peer1".to_string()).await.is_ok());
        assert_eq!(handler.get_active_peers().len(), 0);
        assert!(!handler.is_connected("peer1"));
    ...
    async fn test_protocol_manager() {
        let config = ProtocolConfig::default();
        let mut manager = ProtocolManager::new(config);
        ...
        assert!(manager.add_protocol(custom_config).is_ok());

        // Test connection establishment
        assert!(manager
            .establish_connection("peer1".to_string(), None)
            .await
            .is_ok());
        assert!(manager
            .establish_connection("peer2".to_string(), Some("custom".to_string()))
            .await
            .is_ok());

        // Test getting active peers
        let active_peers = manager.get_active_peers();
        assert_eq!(active_peers.get("savitri-p2p").unwrap().len(), 1);
        assert_eq!(active_peers.get("custom").unwrap().len(), 1);

        // Test getting stats
        let stats = manager.get_all_stats();
        assert!(stats.contains_key("savitri-p2p"));
        assert!(stats.contains_key("custom"));
    }
    ...
    fn test_message_size_validation() {
        ...
        assert!(handler
            .send_message("peer1".to_string(), large_data)
            .await
            .is_err());
    }
