...
mod tests {
    ...
    #[test]
    fn test_rpc_client_endpoint_formatting() {
        // Without trailing slash
        let client = RpcClient::from_url("http://localhost:8545").unwrap();
        assert_eq!(client.endpoint, "http://localhost:8545/rpc");
    ...
    #[test]
    fn test_https_enforcement_rejects_remote_http() {
        // Remote HTTP should be rejected
        let result = RpcClient::from_url("http://example.com:8545");
        assert!(result.is_err());

        let result = RpcClient::from_url("http://192.168.1.1:8545");
        assert!(result.is_err());
    }

    #[test]
    fn test_https_enforcement_allows_localhost() {
        // Localhost HTTP should be allowed
        assert!(RpcClient::from_url("http://localhost:8545").is_ok());
        assert!(RpcClient::from_url("http://127.0.0.1:8545").is_ok());
        assert!(RpcClient::from_url("http://[::1]:8545").is_ok());
    }

    #[test]
    fn test_https_enforcement_allows_https() {
        assert!(RpcClient::from_url("https://rpc.savitri.network").is_ok());
    }

    #[test]
    fn test_allow_insecure_override() {
        // With allow_insecure, remote HTTP should work
        let config = RpcClientConfig {
            url: "http://example.com:8545".to_string(),
            timeout: Some(30),
            allow_insecure: true,
        };
        assert!(RpcClient::new(config).is_ok());
    }
