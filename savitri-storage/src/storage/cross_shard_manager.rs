...
mod tests {
    ...
    #[test]
    fn test_transaction_manager_creation() -> Result<()> {
        let temp_dir = TempDir::new()?;
        let storage = Storage::<RocksDb>::new(temp_dir.path())?;
        let shard_aware = Arc::new(crate::storage::shard_aware::ShardAwareStorage::<RocksDb>::new(storage));
        let config = ShardingConfig::with_num_shards(4);
        
        let manager = CrossShardTransactionManager::new(shard_aware, config);
    ...
    #[test]
    fn test_single_shard_transaction() -> Result<()> {
        let temp_dir = TempDir::new()?;
        let storage = Storage::<RocksDb>::new(temp_dir.path())?;
        let shard_aware = Arc::new(crate::storage::shard_aware::ShardAwareStorage::<RocksDb>::new(storage));
        let config = ShardingConfig::with_num_shards(4);
        
        let manager = CrossShardTransactionManager::new(shard_aware, config.clone());
        
        // Create single-shard transaction
        let participants = vec![TransactionParticipant {
            address: b"test_address".to_vec(),
            shard_id: config.shard_for_address(b"test_address"),
            operation: TransactionOperation::Write,
            data: Some(b"test_data".to_vec()),
        }];
        
        let result = manager.execute_transaction(participants)?;
        ...
        assert_eq!(metrics.single_shard_transactions, 1);
        
        Ok(())
    }

    #[test]
    fn test_cross_shard_transaction() -> Result<()> {
        let temp_dir = TempDir::new()?;
        let storage = Storage::<RocksDb>::new(temp_dir.path())?;
        let shard_aware = Arc::new(crate::storage::shard_aware::ShardAwareStorage::<RocksDb>::new(storage));
        let config = ShardingConfig::with_num_shards(4);
        ...
        let addr1 = b"address_shard_0".to_vec();
        let addr2 = b"address_shard_1".to_vec();
        
        let participants = vec![
            TransactionParticipant {
                address: addr1.clone(),
                shard_id: config.shard_for_address(&addr1),
                operation: TransactionOperation::Write,
                data: Some(b"data1".to_vec()),
            },
            TransactionParticipant {
                address: addr2.clone(),
                shard_id: config.shard_for_address(&addr2),
                operation: TransactionOperation::Write,
        ...
        assert_eq!(result.state, TransactionState::Committed);
        assert_eq!(result.operations_executed, 2);
        assert!(result.error_message.is_none());
...
// Add this to support rand::random in tests
mod rand {
    pub fn random<T>() -> T 
    where 
        T: From<u32>
