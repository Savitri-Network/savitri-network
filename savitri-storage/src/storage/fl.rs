...
mod tests {
    ...
    #[test]
    fn test_fl_policy_default() {
        let policy = FlPolicy::default();
        assert_eq!(policy.fee_treasury_bps, 100);
        assert_eq!(policy.max_models, 100);
        assert_eq!(policy.min_contributions_per_round, 5);
        assert_eq!(policy.max_contributions_per_round, 100);
        assert_eq!(policy.reward_per_contribution, 1000000);
    }
    ...
    fn test_model_serialization() {
        let model = FlModel {
            ...
            metadata: vec![4, 5, 6],
            training_data_hash: Some([7; 32]),
            accuracy_metrics: None,
        };

        let serialized = bincode::serialize(&model).unwrap();
        let deserialized: FlModel = bincode::deserialize(&serialized).unwrap();
    ...
    fn test_reward_id_generation() {
        ...
        let id1 = FlStorage::generate_reward_id(&recipient, round_id, timestamp);
        let id2 = FlStorage::generate_reward_id(&recipient, round_id, timestamp);

        assert_eq!(id1, id2); // Should be deterministic

        let id3 = FlStorage::generate_reward_id(&recipient, round_id + 1, timestamp);
