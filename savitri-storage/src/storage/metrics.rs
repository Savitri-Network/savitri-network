...
mod tests {
    ...
    #[test]
    fn test_storage_metrics_basic() {
        // Test read latency
        record_read_latency_ms(10);
        record_read_latency_ms(15);
        
        // Test write latency
        record_write_latency_ms(20);
        record_write_latency_ms(25);
        ...
        record_compaction_duration_sec(30);
        
        // Test disk usage
        set_disk_usage_bytes(1024 * 1024 * 1024); // 1GB
        
        // Test key count
        set_key_count(1000);
        assert_eq!(get_key_count(), 1000);
        increment_key_count();
        assert_eq!(get_key_count(), 1001);
        decrement_key_count();
        assert_eq!(get_key_count(), 1000);
        
        println!("✅ Storage metrics basic functionality test passed");
    }
    
    #[test]
    fn test_measure_read() -> Result<(), anyhow::Error> {
        let result: Result<String, _> = measure_read(|| {
            Ok("data read".to_string())
        });
        
        assert!(result.is_ok());
        assert_eq!(result?, "data read");
        
        println!("✅ Measure read test passed");
        Ok(())
    }
    
    #[test]
    fn test_measure_write() -> Result<(), anyhow::Error> {
        let result: Result<String, _> = measure_write(|| {
            Ok("data written".to_string())
        });
        
        assert!(result.is_ok());
        assert_eq!(result?, "data written");
        
        println!("✅ Measure write test passed");
        Ok(())
    }
    
    #[test]
    fn test_measure_compaction() -> Result<(), anyhow::Error> {
        let result: Result<String, _> = measure_compaction(|| {
            Ok("compaction completed".to_string())
        });
        
        assert!(result.is_ok());
        assert_eq!(result?, "compaction completed");
        
        println!("✅ Measure compaction test passed");
        Ok(())
    }
    ...
    fn test_cache_hit_rate_calculation() {
        ...
        record_cache_hit();
        record_cache_hit();
        assert_eq!(get_cache_hit_rate(), 100.0);
        ...
        println!("✅ Cache hit rate calculation test passed");
    }
}
