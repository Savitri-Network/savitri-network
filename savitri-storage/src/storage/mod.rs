...
mod tests {
    ...
    fn test_column_families() -> Result<()> {
        ...
        storage.put_cf(CF_BLOCKS, b"block_1", b"block_data")?;
        let block_data = storage.get_cf(CF_BLOCKS, b"block_1")?;
        assert_eq!(block_data, Some(b"block_data".to_vec()));

        storage.put_cf(CF_TRANSACTIONS, b"tx_1", b"tx_data")?;
        let tx_data = storage.get_cf(CF_TRANSACTIONS, b"tx_1")?;
        assert_eq!(tx_data, Some(b"tx_data".to_vec()));

        storage.put_cf(CF_ACCOUNTS, b"acct_1", b"acct_data")?;
        let acct_data = storage.get_cf(CF_ACCOUNTS, b"acct_1")?;
        assert_eq!(acct_data, Some(b"acct_data".to_vec()));
    ...
    #[cfg(feature = "rocksdb")]
    #[test]
    fn test_reopen_adds_missing_accounts_cf() -> Result<()> {
        let temp_dir = TempDir::new()?;
        ...
        {
            let _legacy_db = DB::open_cf_descriptors(&opts, &db_path, descriptors)?;
        }

        let storage = Storage::new(temp_dir.path())?;
        storage.put_cf(CF_ACCOUNTS, b"acct_legacy", b"acct_data")?;
        let acct_data = storage.get_cf(CF_ACCOUNTS, b"acct_legacy")?;
        assert_eq!(acct_data, Some(b"acct_data".to_vec()));
    ...
    fn test_batch_operations() -> Result<()> {
        ...
        batch.put_cf(CF_BLOCKS, b"block_1", b"block_data")?;
        batch.commit()?;
        ...
        assert_eq!(
            storage.get_cf(CF_BLOCKS, b"block_1")?,
    ...
    fn test_performance_stats() -> Result<()> {
        ...
        // Reset stats
        storage.reset_stats();
        let stats_reset = storage.get_stats();
        assert_eq!(stats_reset.read_count, 0);
        assert_eq!(stats_reset.write_count, 0);
        assert_eq!(stats_reset.batch_count, 0);

        Ok(())
    }

    #[test]
    fn test_column_family_handles() -> Result<()> {
        let temp_dir = TempDir::new()?;
        let storage = Storage::new(temp_dir.path())?;

        // Test getting column family handles
        let cf_blocks = storage.cf(CF_BLOCKS)?;
        let cf_transactions = storage.cf(CF_TRANSACTIONS)?;

        #[cfg(feature = "rocksdb")]
        {
            // Test using column family handles directly
            storage.put_cf_handle(&cf_blocks, b"block_1", b"block_data")?;
            let block_data = storage.get_cf_handle(&cf_blocks, b"block_1")?;
            assert_eq!(block_data, Some(b"block_data".to_vec()));

            storage.put_cf_handle(&cf_transactions, b"tx_1", b"tx_data")?;
            let tx_data = storage.get_cf_handle(&cf_transactions, b"tx_1")?;
            assert_eq!(tx_data, Some(b"tx_data".to_vec()));
        }

        #[cfg(not(feature = "rocksdb"))]
        {
            // Test using mock column family handles
            assert_eq!(cf_blocks.name(), CF_BLOCKS);
            assert_eq!(cf_transactions.name(), CF_TRANSACTIONS);
        ...
        let invalid_cf = storage.cf("invalid_cf");
        assert!(invalid_cf.is_err());
