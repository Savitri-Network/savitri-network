...
mod tests {
    ...
    fn test_rate_limit_tracking() {
        let mut tracking = ConnectorRateLimitTracking::new(1000);

        // Should allow first request
        assert!(tracking.is_request_allowed(1001, 10, 1));
        assert_eq!(tracking.ingest_count, 1);

        // Should allow more requests up to limit
        for i in 2..=10 {
            assert!(tracking.is_request_allowed(1000 + i, 10, 1));
        }

        // Should deny request over limit
        assert!(!tracking.is_request_allowed(1011, 10, 1));
    }

    #[test]
    fn test_oracle_anchor() {
        let anchor = OracleAnchor::new(vec![1, 2, 3], 100, vec![4, 5, 6], 12345, vec![7, 8, 9]);

        assert!(anchor.is_valid());
        assert_eq!(anchor.block_height, 100);
        assert_eq!(anchor.timestamp, 12345);
    }

    #[test]
    fn test_schema_serialization() {
        let schema = OracleSchema {
            schema_id: vec![1, 2, 3],
            ...
            created_at: 12345,
            created_by: vec![9, 8, 7],
        };

        let serialized = bincode::serialize(&schema).unwrap();
        let deserialized: OracleSchema = bincode::deserialize(&serialized).unwrap();
