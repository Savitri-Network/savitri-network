...
mod tests {
    ...
    #[test]
    fn test_pou_score_storage() {
        let temp_dir = TempDir::new().unwrap();
        let storage = Storage<RocksDb>::new(temp_dir.path()).unwrap();

        let node_id = [1u8; 32];
        let epoch = 1;
        let components = ScoreComponents::new(
            FixedPoint::from_ratio(95, 100).unwrap(),
            FixedPoint::from_ratio(90, 100).unwrap(),
            FixedPoint::from_ratio(98, 100).unwrap(),
            FixedPoint::from_ratio(85, 100).unwrap(),
        );
        let timestamp = 1234567890;

        // Store
        storage
            .put_pou_score(&node_id, epoch, &components, timestamp)
            .unwrap();

        // Retrieve
        let entry = storage.get_pou_score(&node_id, epoch).unwrap().unwrap();
        assert_eq!(entry.node_id, node_id);
        assert_eq!(entry.epoch, epoch);
        assert_eq!(entry.components.u, 950_000);
        assert_eq!(entry.timestamp, timestamp);
    }

    #[test]
    fn test_pou_history_storage() {
        let temp_dir = TempDir::new().unwrap();
        let storage = Storage<RocksDb>::new(temp_dir.path()).unwrap();

        let node_id = [2u8; 32];
        let u_history = FixedPoint::from_ratio(8, 10).unwrap();

        // Store
        storage
            .put_pou_history(&node_id, u_history, 0, 3, 0, 1)
            .unwrap();

        // Retrieve
        let entry = storage.get_pou_history(&node_id).unwrap().unwrap();
        assert_eq!(entry.node_id, node_id);
        assert_eq!(entry.u_history, 800_000);
        assert_eq!(entry.longest_miss_streak, 3);
    }

    #[test]
    fn test_pou_score_retention_keeps_only_recent() {
        let temp_dir = TempDir::new().unwrap();
        let storage = Storage<RocksDb>::new(temp_dir.path()).unwrap();

        let node_id = [9u8; 32];
        let components = ScoreComponents::new(
            FixedPoint::from_ratio(1, 1).unwrap(),
            FixedPoint::from_ratio(1, 1).unwrap(),
            FixedPoint::from_ratio(1, 1).unwrap(),
            FixedPoint::from_ratio(1, 1).unwrap(),
        );

        for epoch in 0u64..120 {
            storage
                .put_pou_score(&node_id, epoch, &components, epoch)
                .unwrap();
        }

        storage.purge_old_pou_scores(119, 100).unwrap();

        // Oldest epochs should be pruned
        assert!(storage.get_pou_score(&node_id, 0).unwrap().is_none());
        assert!(storage.get_pou_score(&node_id, 10).unwrap().is_none());

        // Recent epochs remain
        assert!(storage.get_pou_score(&node_id, 119).unwrap().is_some());
        assert!(storage.get_pou_score(&node_id, 100).unwrap().is_some());
    }
}

// LightNodeInfo storage methods
impl Storage<RocksDb> {
    /// Store light node information
    pub fn put_light_node_info(&self, info: &LightNodeInfo) -> Result<()> {
        let key = light_node_info_key(&info.pubkey);
        let value = bincode::serialize(info)?;
        self.put_cf(CF_POU_SCORES, key, value)?;
        Ok(())
    }

    /// Retrieve light node information
    pub fn get_light_node_info(&self, pubkey: &NodeId) -> Result<Option<LightNodeInfo>> {
        let key = light_node_info_key(pubkey);
        if let Some(value) = self.get_cf(CF_POU_SCORES, key)? {
            let info: LightNodeInfo = crate::safe_deserialize(&value[..])?;
            Ok(Some(info))
        } else {
            Ok(None)
        }
    ...
    /// Get all light nodes in a specific geographic region
    pub fn get_light_nodes_by_region(&self, region: &GeographicRegion) -> Result<Vec<LightNodeInfo>> {
        let cf = self.cf(CF_POU_SCORES)?;
        let iter = self.db.iterator_cf(&cf, IteratorMode::Start);
        ...
        for item in iter {
            ...
            // Only process light node info keys (prefix with "lni")
            if key.len() >= 3 && &key[0..3] == b"lni" {
                let info: LightNodeInfo = crate::safe_deserialize(&value[..])?;
                if info.geographic_region == *region {
                    nodes.push(info);
                }
            }
        }
    ...
    /// Get all eligible light nodes for proposer election
    pub fn get_eligible_light_nodes(&self, min_score: u32) -> Result<Vec<LightNodeInfo>> {
        let cf = self.cf(CF_POU_SCORES)?;
        let iter = self.db.iterator_cf(&cf, IteratorMode::Start);
        ...
        for item in iter {
            ...
            if key.len() >= 3 && &key[0..3] == b"lni" {
                let info: LightNodeInfo = crate::safe_deserialize(&value[..])?;
                if info.is_eligible && info.pou_score >= min_score {
                    nodes.push(info);
                }
            }
        }
        
        // Sort by PoU score (descending)
        nodes.sort_by(|a, b| b.pou_score.cmp(&a.pou_score));
        Ok(nodes)
    }

    /// Update light node eligibility based on current requirements
    pub fn update_light_node_eligibility(&self, pubkey: &NodeId, current_epoch: u64, min_stake: u64) -> Result<()> {
        if let Some(mut info) = self.get_light_node_info(pubkey)? {
            // Check eligibility requirements
            let meets_stake = info.stake_amount >= min_stake;
            let meets_uptime = info.uptime_24h >= 0.8; // 80% uptime requirement
            let meets_score = info.pou_score >= 1000; // Minimum 10% score
            
            info.is_eligible = meets_stake && meets_uptime && meets_score;
            info.current_epoch = current_epoch;
            
            // Store updated info
            self.put_light_node_info(&info)?;
        }
...
/// Generate storage key for light node info
fn light_node_info_key(pubkey: &NodeId) -> Vec<u8> {
    let mut key = Vec::with_capacity(3 + 32);
    key.extend_from_slice(b"lni"); // "light node info" prefix
