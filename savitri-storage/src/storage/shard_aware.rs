...
mod tests {
    ...
    #[test]
    fn test_shard_aware_storage_creation() -> Result<()> {
        let temp_dir = TempDir::new()?;
        let storage = Storage::<RocksDb>::new(temp_dir.path())?;
        let shard_aware = ShardAwareStorage::<RocksDb>::new(storage);
        
        // Verify locks are created
        assert_eq!(shard_aware.account_shard_locks.len(), 8);
        assert_eq!(shard_aware.contract_shard_locks.len(), 8);
        
        Ok(())
    }

    #[test]
    fn test_shard_lock_acquisition() -> Result<()> {
        let temp_dir = TempDir::new()?;
        let storage = Storage::<RocksDb>::new(temp_dir.path())?;
        let shard_aware = ShardAwareStorage::<RocksDb>::new(storage);
        
        // Test lock acquisition
        let result = shard_aware.acquire_account_shard_lock(0);
        assert!(result.acquired);
        assert!(result.acquisition_time_ns > 0);
        
        // Test invalid shard
        let result = shard_aware.acquire_account_shard_lock(99);
        assert!(!result.acquired);
    ...
    #[test]
    fn test_cross_shard_operation_context() -> Result<()> {
        let temp_dir = TempDir::new()?;
        let storage = Storage::<RocksDb>::new(temp_dir.path())?;
        let shard_aware = ShardAwareStorage::<RocksDb>::new(storage);
        let config = ShardingConfig::with_num_shards(4);
        
        // Create addresses for different shards
        let addr1 = b"address1_for_shard_0";
        let addr2 = b"address2_for_shard_1";
        let addr3 = b"address3_for_shard_0";
        
        let addresses = vec![addr1, addr2, addr3];
        let context = shard_aware.create_operation_context(&addresses, &config);
        
        assert!(context.is_cross_shard);
        assert_eq!(context.involved_shards.len(), 2); // shards 0 and 1
    ...
    fn test_performance_metrics() -> Result<()> {
        ...
        // Acquire some locks to generate metrics
        shard_aware.acquire_account_shard_lock(0);
        shard_aware.acquire_account_shard_lock(1);
        shard_aware.acquire_global_lock();
        
        let metrics = shard_aware.get_metrics();
        assert_eq!(metrics.shard_lock_acquisitions, 2);
        assert_eq!(metrics.global_lock_acquisitions, 1);
        
        let stats = shard_aware.get_lock_statistics();
        assert!(stats.contains_key("shard_lock_acquisitions"));
        assert!(stats.contains_key("global_lock_acquisitions"));
    ...
    fn test_performance_analysis() -> Result<()> {
        ...
        let recommendations = shard_aware.analyze_performance();
        assert!(!recommendations.is_empty());
        assert!(recommendations[0].contains("optimal"));
        
        Ok(())
    }
}

impl<S: StorageTrait> ShardAwareStorage<S> {
    /// Get column family name from ColumnFamily handle
    /// This is a helper function to resolve CF names from handles
    fn get_cf_name(&self, cf: &rocksdb::ColumnFamily) -> Result<String> {
        // Try to get the name from known column families
        let known_cfs = vec![
            ("default", self.storage.cf("default")),
            ("blocks", self.storage.cf("blocks")),
            ("transactions", self.storage.cf("transactions")),
            ("state", self.storage.cf("state")),
            ("metadata", self.storage.cf("metadata")),
            ("accounts", self.storage.cf("accounts")),
        ];
        
        for (name, known_cf) in known_cfs {
            if let Ok(known_cf) = known_cf {
                // Compare the memory addresses to see if they're the same CF
                if std::ptr::eq(cf, known_cf) {
                    return Ok(name.to_string());
                }
            }
        }
        
        // If we can't find it in known CFs, try to get it from the DB
        #[cfg(feature = "rocksdb")]
        {
            // Use the DB's internal method to get CF name
            // This is a fallback method that may not work in all cases
            let cf_names = vec!["default", "blocks", "transactions", "state", "metadata", "accounts"];
            for cf_name in cf_names {
                if let Ok(handle) = self.storage.cf(cf_name) {
                    if std::ptr::eq(cf, &handle) {
        ...
        warn!("Could not resolve column family name, using 'default'");
